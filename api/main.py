...
@app.get("/order_details/{order_id}", response_model=schemas.OrderDetail, tags=["Order_Details"])
...
def update_one_order_detail(order_id: int, order: schemas.OrderUpdate, db: Session = Depends(get_db)):
    order_db = orders.read_one(db, order_id=order_id)
    if order_db is None:
        raise HTTPException(status_code=404, detail="User not found")
    return orders.update(db=db, order=order, order_id=order_id)
