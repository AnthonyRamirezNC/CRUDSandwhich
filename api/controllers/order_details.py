...
def update(db: Session, order_detail, order_detail_id):
    db_order_detail = db.query(models.OrderDetail).filter(models.OrderDetail.id == order_detail_id)
    update_order_detail = order_detail.dict(exclude_unset=True)
    db_order_detail.update(update_order_detail, synchronize_session=False)
    db.commit()
